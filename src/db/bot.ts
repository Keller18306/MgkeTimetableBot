...
import { BotServiceName } from "../services/bots/abstract";
import { addslashes } from "../utils";

export function getBotUser(table: string, service: BotServiceName, peerId: string | number): any {
    return db.prepare(
        `SELECT * FROM ${table} JOIN chat_options ON ${table}.id = chat_options.id AND chat_options.service = ? WHERE ${table}.peerId = ?`
    ).get(service, peerId);
}

export function addNewBotUser(table: string, service: BotServiceName, peerId: string | number, accepted: boolean, allowSendMess: boolean) {
...
export function updateChatOptionsKeyByPeerId(table: string, service: BotServiceName, key: string, value: any, peerId: string | number) {
...
export function getGroupsChats(table: string, service: BotServiceName, groups: string | string[]): any {
    return db.prepare(
...
export function getTeachersChats(table: string, service: BotServiceName, teachers: string | string[]): any {
    return db.prepare(
...
export function getGroupsNoticeNextWeekChats(table: string, service: BotServiceName, groups: string | string[]): any {
    return db.prepare(
...
export function getTeachersNoticeNextWeekChats(table: string, service: BotServiceName, teachers: string | string[]): any {
    return db.prepare(
...
export function getDistributionChats(table: string, service: BotServiceName): any {
    return db.prepare(
...
export function getNoticeErrorsChats(table: string, service: BotServiceName): any {
    return db.prepare(
...
export function getAllowSendMessCount(service: BotServiceName): number {
    return (db.prepare("SELECT COUNT(*) as `count` FROM `chat_options` WHERE `service` = ? AND `allowSendMess` = 1").get(service) as any).count;
}


export function getChatLessonAliases(service: BotServiceName, chatId: string | number): { [key: string]: string } {
