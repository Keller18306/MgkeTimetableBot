...
export function addValueIntoStorageByKey(storage: string, key: string, value: string, time: number, expires: number): void {
    db.prepare(
        'INSERT INTO storage (`storage`, `key`, `value`, `time`, `expires`) VALUES (:storage, :key, :value, :time, :expires) ON CONFLICT(`storage`, `key`) DO UPDATE SET value = :value, time = :time, expires = :expires')
